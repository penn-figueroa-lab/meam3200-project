...
%% ================== SYSTEM PARAMETERS ==================
global total_I lever2motor b_bearing K_T K_tau g motor_weight
...
lever_I  = lever_weight * (lever_size(1)^2 + lever_size(2)^2) / 12; % [kg*m^2]
motor_I  = motor_weight * (lever2motor(1)^2 + lever2motor(2)^2);    % [kg*m^2]
total_I  = lever_I + motor_I;                                       % total inertia [kg*m^2]
...
%% ================== PID PARAMETERS ==================
% s = tf('s');
% linearized_tf = K * L / (total_I*s^2+b_bearing*s-motor_weight*norm(lever2motor)*sin(atan2(lever2motor(2), lever2motor(1))));
% C = pidTuner(linearized_tf,'pid');
Kp = 1.3888; Ki = 1.511; Kd = 0.31914;
previousError = 0; 
integral = 0;
...
for k = 1:steps
    ...
    derivative = (k==1)*0 + (k>1)*(error - previousError)/dt;
    u = Kp*error + Ki*integral + Kd*derivative;
    u = min(max(u,0),3.2);  % clip
    previousError = error;
    ...
    if enable_perturbation
        ...
        if perturb_timer > 0
            state = state + dt*[0; L*perturb_strength/total_I];
            perturb_flag = 1;
...
function dx = dynamics(x,u)
    ...
    theta     = x(1);
    theta_dot = x(2);
    psi = atan2(lever2motor(2), lever2motor(1));
    tau_motor   = -motor_weight * g * norm(lever2motor) * cos(theta + psi); % [N*m]
    tau_bearing = -b_bearing * theta_dot;                                     % [N*m]
    tau_thrust  = K_T*K_tau*u*lever2motor(1);                                 % [N*m]
    theta_ddot = (tau_motor + tau_bearing + tau_thrust)/total_I;              % [rad/s^2]
